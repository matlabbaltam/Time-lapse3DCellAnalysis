...
def rotations24(polycube):
    ...
    # 4 rotations about axis 0
	results = torch.cat((results,rotations4(rot90(polycube, 2, dims=(2,4)), (3,4))),0)

    # rotate 90 or 270 about axis 1, now shape is pointing in axis 2
    # 8 rotations about axis 2
	results = torch.cat((results,  rotations4(rot90(polycube, dims=(2,4)), (2,3))),0)
	results = torch.cat((results,  rotations4(rot90(polycube, -1, dims=(2,4)), (2,3))),0)

    # rotate about axis 2, now shape is pointing in axis 1
    # 8 rotations about axis 1
	results = torch.cat((results,rotations4(rot90(polycube, dims=(2,3)), (2,4))),0)
	results = torch.cat((results,  rotations4(rot90(polycube, -1, dims=(2,3)), (2,4))),0)
	return results
...
class groupConv3d(nn.Conv3d):
    def __init__(
        ...
        padding_ = padding if isinstance(padding, str) else tuple([padding,padding,padding])
    ...
    def _conv_forward(self, input, weight, bias):
        ...
        return F.conv3d(
            input, group_weight, bias, self.stride, self.padding, self.dilation, self.groups
            )
